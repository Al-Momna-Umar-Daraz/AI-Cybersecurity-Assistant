(function () {
    ...
    window.runEncryptionTool = function runEncryptionTool() {
        ...
        apiFetchJson('/api/encryption-tool', {
            ...
            .then(function (data) {
                ...
                var outputLabel = byId('encryptionOutputLabel');
                if (outputLabel) outputLabel.textContent = actionKey === 'decrypt_text' ? 'Your Message' : 'Output';
                if (actionKey === 'encrypt_text') {
                    ...
                    if (outputNode) {
                        outputNode.focus();
                        outputNode.setSelectionRange(0, 0);
                    }
                } else {
                    syncEncryptionDecryptPreview('');
                }
    ...
    function renderLocalFaceList(data) {
        ...
        cachedLocalFaceItems = items.slice();
        if (!items.length) {
    ...
    window.refreshLocalFaceProfiles = function refreshLocalFaceProfiles(silent) {
        setFaceActionBusy('refreshFaceIntelBtn', true, '<i class="fas fa-spinner fa-spin"></i> Refreshing...');
        return apiFetchJson('/api/face-intel/local-faces')
            .then(function (data) {
                const payload = data || {};
                const count = Number(payload.count || 0);
                renderLocalFaceList(payload);
                setFaceInputNotice(
                    count
                        ? ('Saved face database refreshed. ' + count + ' profile(s) are ready.')
            ...
            .catch(function (err) {
                const message = (err && err.message) ? err.message : 'Could not refresh saved face profiles.';
                setFaceInputNotice(message, 'warning');
                if (!silent) showAppDialog(message, 'Face Database');
                throw err;
            })
            .finally(function () {
    ...
    function initFaceIntelPage() {
        ...
        window.refreshLocalFaceProfiles(true).catch(function () { });
    }
